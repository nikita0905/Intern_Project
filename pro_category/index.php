...
if(isset($_GET['formact']) && $_GET['formact']=='search'){
    $cond.=" and `name` Like '%".$_GET['keyword']."%' ";
}
...
if (isset($_POST['formact']) && $_POST['formact'] == 'del') {
    ...
    foreach ($checkrec as $key => $val) {
        mysql_query("delete from `category` where `id`=" . $val);
    }
    $_SESSION['msg'] ="Categories deleted successfully";
    header("Location:index.php");
    exit();
}
?>
...
{ echo 'active'; } ?>"><a href='#'><span>IMAGE CATEGORY MANAGER</span></a>
      <ul style="list-style: none; ">
         ...
         <li><a href='<?php echo $base_path;?>img_category/addcat.php'><span>ADD IMAGE CATEGORY</span></a></li>
                    ...
                    <div class="row">
                    <ol class="breadcrumb">
                                    <li><a href="#">Product Category Manager</a></li>
   <li><a href="index.php">View Product Categories list</a></li>
   <li><a href="addcat.php">Add Product category</a></li>
                        ...
                        <div class="table-responsive">
                            <div class="panel panel-primary">
                                ...
                                <div class="panel-heading">
                                  <div class='row'>  <div class="col-md-90 col-sm-90 col-xs-90"><span class="glyphicon glyphicon-shopping-cart"></span>&nbsp;&nbsp;Categories</div>
                                
                                <div class="col-md-10 col-sm-10 col-xs-10">
                                    <a href="addcat.php" class="btn btn-default" > <span class="glyphicon glyphicon-plus"></span></a>
                                </div>
                                  </div>
                            </div>
                            ...
                            <table class="table table-bordered">
                                ...
                                <?php
                                $pagesize=PAGESIZE;
                                $rs = mysql_query("select * from `category` where $cond and `type`='product'");
                                $num = mysql_num_rows($rs);
                                $pageindex=isset($_GET['pageindex'])? $_GET['pageindex']:0;
                                $totalpage=  ceil($num/$pagesize);
                                if($num){
                                    $rspage = mysql_query("select * from `category` where $cond and `type`='product' limit ".($pageindex*$pagesize).",".$pagesize);
                                    ...
                                    <td class="tdaction">
                                        <a href="addcat.php?todo=edit&id=<?php echo $row['id'] ?>"><span class="glyphicon glyphicon-edit"></span></a>
                                ...
                                ?>
                                <!-- <tr><td colspan="5"> <div class="col-md-offset-10 col-md-2"><input type="hidden" name="formact" id="formact" value="" />
                                                        <button type="button" id="deleteAll" class="btn btn-sm btn-danger"><span class="glyphicon glyphicon-remove"></span>&nbsp;&nbsp;Delete All</button></td> </div></tr> -->
                            </tbody>
                            </table>
