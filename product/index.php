...
if(isset($_GET['todo']) && $_GET['todo']=='addimg'){
header("Location: pro_img.php");
exit;
}
...
if (isset($_POST['formact']) && $_POST['formact'] == 'del') {
    ...
    foreach ($checkrec as $key => $val) {
        mysql_query("delete from `product` where `id`=" . $val);
    }
                        ...
                        <div class="table-responsive">
                            <div class="panel panel-primary">
                                ...
                                <div class="panel-heading">
                                  <div class='row'>  <div class="col-md-90 col-sm-90 col-xs-90"><span class="glyphicon glyphicon-credit-card"></span>&nbsp;&nbsp;Products</div>
                                
                                <div class="col-md-10 col-sm-10 col-xs-10">
                                    <a href="addproduct.php" class="btn btn-default" > <span class="glyphicon glyphicon-plus"></span></a>
                                </div>
                                  </div>
                            </div>
                            ...
                            <table class="table table-bordered">
                                ...
                                <?php
                                $pagesize=PAGESIZE;
                                $rs = mysql_query("select * from `product` where $cond");
                                $num = mysql_num_rows($rs);
                                $pageindex=isset($_GET['pageindex'])? $_GET['pageindex']:0;
                                $totalpage=  ceil($num/$pagesize);
                                if($num){
                                    $rspage = mysql_query("select * from `product` where $cond limit ".($pageindex*$pagesize).",".$pagesize);
                                    ...
                                    <td class="tdaction">
                                        <a href="addproduct.php?todo=edit&id=<?php echo $row['id'] ?>"><span class="glyphicon glyphicon-edit"></span></a>
                                        <a href="index.php?todo=delete&id=<?php echo $row['id'] ?>" onclick="return deleteRec();"><span class="glyphicon glyphicon-trash"></span></a> 
                                        <a href="prod_img.php?product_id=<?php echo $row['id'] ?>"><span class="glyphicon glyphicon-picture"></span></a> 
